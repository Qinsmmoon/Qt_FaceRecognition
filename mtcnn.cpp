...
//加载模型
mtcnn::mtcnn() {
    std::string param1 = project_path + "/models/mtcnn/det1.param"; // Pnet
    std::string model1 = project_path + "/models/mtcnn/det1.bin";
    std::string param2 = project_path + "/models/mtcnn/det2.param"; // Rnet
    std::string model2 = project_path + "/models/mtcnn/det2.bin";
    std::string param3 = project_path + "/models/mtcnn/det3.param"; // Onet
...
//nms非极大值抑制
void mtcnn::nms(std::vector<Bbox>& boundingBox_, std::vector<orderScore>& bboxScore_, const float overlap_threshold, std::string modelname) {
        ...
        for (int num = 0; num < boundingBox_.size(); num++) {
            if (boundingBox_.at(num).exist) {
                ...
                if (IOU > overlap_threshold) {
                    ...
                    for (std::vector<orderScore>::iterator it = bboxScore_.begin(); it != bboxScore_.end(); it++) {
                        if ((*it).oriOrder == num) {
                            (*it).oriOrder = -1;
                            break;
                        }
...
void mtcnn::refineAndSquareBbox(std::vector<Bbox>& vecBbox, const int& height, const int& width) {
    if (vecBbox.empty()) {
...
void mtcnn::detect(ncnn::Mat& img_, std::vector<Bbox>& finalBbox_) {
    ...
    int MIN_DET_SIZE = 12;
    int minsize = 90; // 最小可检测图像，该值大小，控制图像金字塔的阶层数，越小，阶层越多，计算越多。
    ...
    for (size_t i = 0; i < scales_.size(); i++) {
        ...
        ncnn::Mat in;
        resize_bilinear(img_, in, ws, hs); // 一次次生成图像金字塔中的一层图
        ...
        for (std::vector<Bbox>::iterator it = boundingBox_.begin(); it != boundingBox_.end(); it++) {
            if ((*it).exist) {
...
std::vector<Bbox> detect_mtcnn(const cv::Mat& cv_img)
{
    ...
    ncnn::Mat ncnn_img2 = bgr2rgb(ncnn_img);
    struct timeval  tv1, tv2;
    struct timezone tz1, tz2;

    gettimeofday(&tv1, &tz1);
    std::vector<Bbox> finalBbox;
    mm.detect(ncnn_img2, finalBbox);
    gettimeofday(&tv2, &tz2);
    detectTime = getElapse(&tv1, &tv2);
